...
impl ObserverCapability for StubObserver {
    async fn observe(
        ...
        for (k, &v) in raw_telemetry {
            // Default to 1-D observation; can be extended to multi-D telemetry later
...
/// Multi-dimensional stub example (2-D) for demonstration.
...
impl ObserverCapability for MultiDimStubObserver {
    async fn observe(
        ...
        // Example: treat temperature + humidity as a 2-D state if both present
        ...
        // Also keep individual scalar observations
        for (k, &v) in raw_telemetry {
            let est = StateEstimate::scalar(v, 0.25, 0.95, now, "stub_observer", cholesky_floor)?;
            map.insert(k.clone(), est);
        }
