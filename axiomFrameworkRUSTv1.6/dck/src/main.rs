...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for ev in &events {
        println!("Event ID   : {}", ev.event_id);
        println!("  Stage    : {:?}", ev.current_stage);
        println!("  Action   : {:?}", ev.decision_action);
        println!("  Gap      : {:.4}", ev.equivalence_gap);
        println!("  Velocity : {:.4}", ev.computed_velocity);
        println!("  Dim      : {}", ev.projected_state.dim());
        println!();
    }
